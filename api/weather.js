...
router.get("/displayInCelsius", async (req, res, next) => {
    try {
        ...
        const longitude = location.data.results[0].longitude;
        const result = await axios.get
            (`https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&start_date=${date1}&end_date=${date2}&daily=temperature_2m_max&timezone=America%2FNew_York`);
        result
            ? res.status(200).json(result.data)
            : res.status(400).send("result not found");
    } catch (error) {
